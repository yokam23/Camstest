...
public class AddHKAgency extends BaseClass {
	...
	@Test(description = "TC0001::Validate Add new HK agency", groups = { "UI" }, dataProvider = "DataReader")
	public void ValidateAddHKagency(String TCNo, String Description, String CompanyName, String CompanyAddress,
		...
		hkutils.GetPageHeader();
		hkutils.GetAddPageHeader();
		if (Switch.equalsIgnoreCase("AddNew")) {
			hkutils.AddhkAgency(CompanyName, CompanyAddress, State, City, Pincode, ContactPerson, PhoneNo, Emailid,
					MobileNo);
		} else if (Switch.equalsIgnoreCase("ErrorMessage")) {
			hkutils.GetValidationMessage();
		} else if (Switch.equalsIgnoreCase("Duplicate")) {
			hkutils.Validateduplicaterecord(CompanyName, CompanyAddress, State, City, Pincode, ContactPerson, PhoneNo,
					Emailid, MobileNo);
		} else if (Switch.equalsIgnoreCase("Refreshbutton")) {
			hkutils.RefreshAddedDetails(CompanyName, CompanyAddress, State, City, Pincode, ContactPerson, PhoneNo,
					Emailid, MobileNo);
		...
		if(Switch.equalsIgnoreCase("Deletion") || Switch.equalsIgnoreCase("AddNew")) {
			...
			Asserts.assertEqual(MasterDBDetails.GetAgencyDetails(CompanyName, "HKA").get(0).get("Pincode"),Pincode , "Correct Pincode name displayed in database");
			Asserts.assertEqual(MasterDBDetails.GetAgencyDetails(CompanyName, "HKA").get(0).get("ContactPerson"),ContactPerson , "Correct ContactPerson name displayed in database");
			Asserts.assertEqual(MasterDBDetails.GetAgencyDetails(CompanyName, "HKA").get(0).get("PhoneNo"),PhoneNo , "Correct PhoneNo name displayed in database");
			Asserts.assertEqual(MasterDBDetails.GetAgencyDetails(CompanyName, "HKA").get(0).get("MobileNo"),MobileNo , "Correct MobileNo name displayed in database");	
			Asserts.assertEqual(MasterDBDetails.GetAgencyDetails(CompanyName, "HKA").get(0).get("EmailID"),Emailid , "Correct EmailID name displayed in database");
		}
